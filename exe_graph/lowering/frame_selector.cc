...
ge::NodePtr GetOrCreateInnerNetOutput(GraphFrame &frame) {
...
ge::graphStatus GetNewOutIndexes(const ge::NodePtr &init_node, const std::vector<ValueHolderPtr> &init_graph_outputs,
                                 size_t start_index, size_t &new_out_num,
...
std::vector<ValueHolderPtr> FrameSelector::OnMainRoot(const std::function<std::vector<ValueHolderPtr>()> &builder) {
  ...
  if (OnMainRoot(builder, outputs) != ge::GRAPH_SUCCESS) {
    ...
    ScopedCurrentFrame frame_guarder(GetGraphFrames().front().get());
    outputs = builder();
  }
  return outputs;
}
ge::graphStatus FrameSelector::OnMainRoot(const std::function<std::vector<ValueHolderPtr>()> &builder,
  ...
  auto root_frame = GetGraphFrames().begin()->get();
  ...
  ScopedCurrentFrame frame_guarder(main_frame);
  outputs = builder();
  return ge::GRAPH_SUCCESS;
}

std::vector<ValueHolderPtr> ValueHolder::GetLastExecNodes() {
  if (GetGraphFrames().empty()) {
    return {};
  }
  auto frame = GetGraphFrames().begin()->get();
  if (GetGraphFrames().size() > 1U) {
    frame = (GetGraphFrames().begin() + 1)->get();
  }
...
ValueHolderPtr FrameSelector::OnMainRootLast(const std::function<bg::ValueHolderPtr()> &builder) {
  ...
  ScopedCurrentFrame frame_guarder(current_frame);
  auto output = builder();
...
std::vector<ValueHolderPtr> FrameSelector::OnInitRoot(const std::function<std::vector<ValueHolderPtr>()> &builder) {
  ...
  auto ret = OnInitRoot(builder, init_graph_outputs, init_node_outputs);
...
ge::graphStatus FrameSelector::OnInitRoot(const std::function<std::vector<ValueHolderPtr>()> &builder,
  ...
  auto init_node = root_frame->GetExeGraph()->FindFirstNodeMatchType(GetExecuteGraphTypeStr(ExecuteGraphType::kInit));
  ...
  if (!init_graph_outputs.empty()) {
    auto netoutput = GetOrCreateInnerNetOutput(*tmp_init_frame);
    GE_ASSERT_NOTNULL(netoutput);
    auto index = netoutput->GetAllInDataAnchorsSize();

    size_t new_out_num;
    std::map<ValueHolderPtr, size_t> graph_outputs_to_out_index;
    GE_ASSERT_SUCCESS(GetNewOutIndexes(init_node, init_graph_outputs, index, new_out_num, graph_outputs_to_out_index));
...
ValueHolderPtr HolderOnInit(const ValueHolderPtr &holder) {
  ...
  auto holder_graph = holder_node->GetOwnerComputeGraph();
  GE_ASSERT_NOTNULL(holder_graph);
  auto parent_node = holder_graph->GetParentNode();
