...
bool ExpandDimension::GenerateReshapeType(const ge::Format &origin_format, const ge::Format &format,
  ...
  if (origin_dim_size > valid_shape_type.length()) {
    GELOGE(ge::GRAPH_FAILED, "The length of reshape type[%s] is no longer than dim size[%zu]. \
           Can not generate integer reshape type.", valid_shape_type.c_str(), origin_dim_size);
    return false;
  }
