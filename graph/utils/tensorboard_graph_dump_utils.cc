...
namespace ge {
namespace {
void ConvertNodeInput(const ge::NodePtr &node,
...
graphStatus ConvertAttr(const std::map<std::string, AnyValue> &attr_map,
                        domi::tensorflow::NodeDef &node_def) {
...
graphStatus ConvertSubgraphAttr(const ge::NodePtr &node,
                                domi::tensorflow::NodeDef &node_def) {
...
graphStatus ConvertNode(const ge::NodePtr &node,
                        domi::tensorflow::NodeDef &node_def) {
...
void ConvertComputeGraphToGraphDef(
    const ge::ComputeGraph &compute_graph, domi::tensorflow::GraphDef &graph_def) {
...
GE_FUNC_DEV_VISIBILITY GE_FUNC_HOST_VISIBILITY graphStatus TensorBoardGraphDumpUtils::DumpGraph(
    const ge::ComputeGraph &compute_graph, const std::string &suffix) {
  ...
  stream_file_name << "ge_tensorboard_" << std::setw(kDumpGraphIndexWidth) << std::setfill('0') << file_index;
  ...
  if ((proto_file.length()) >= kMaxFileNameLen) {
