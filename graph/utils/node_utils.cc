    ...
    for (auto &sub_graph_name : sub_graph_names) {
      ...
      for (const auto &node_ptr : sub_graph->GetDirectNode()) {
        ...
        if (status != GRAPH_SUCCESS) {
          REPORT_CALL_ERROR("E18888", "GetNodeUnknownShapeStatus failed, node:%s, status:%d",
...
/// @return bool
bool NodeUtils::IsRefFromInput(const OutDataAnchorPtr &out_data_anchor, int32_t &reuse_in_index) {
  if (out_data_anchor == nullptr) {
...
bool NodeUtils::IsNoPaddingRefFromInput(const OutDataAnchorPtr &out_data_anchor, int32_t &reuse_in_index) {
  const auto node = out_data_anchor->GetOwnerNodeBarePtr();
...
bool NodeUtils::HasRefVarAttr(const OutDataAnchorPtr &out_data_anchor, std::string &src_node_name) {
