...
class TensorImpl {
 public:
  ...
  graphStatus SetData(const std::vector<std::string> &data) {
    ...
    StringHead * const string_head = reinterpret_cast<StringHead *>(buff.get());
    char_t *raw_data = static_cast<char_t *>(
        ValueToPtr(PtrToValue(static_cast<void *>(buff.get())) + (data.size() * sizeof(*string_head))));
    uint64_t ptr_size = data.size() * sizeof(StringHead);
    for (size_t i = 0U; i < data.size(); ++i) {
      string_head[i].addr = PtrToValue(static_cast<void *>(raw_data));
