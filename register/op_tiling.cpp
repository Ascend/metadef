...
class AnyVecValue : public AnyValueBase {
 public:
  ...
  DataBuf GetDataBuf() const override {
    return DataBuf(reinterpret_cast<const uint8_t *>(value_.data()),
                   sizeof(VT) * value_.size());
  }
...
struct Getter<T, typename std::enable_if<std::is_integral<T>::value>::type> {
  ...
  static constexpr bool (*list_func)(ge::AttrUtils::ConstAttrHolderAdapter &&,
                                     const string &, vector<int64_t> &) =
      ge::AttrUtils::GetListInt;
};
template <typename T>
struct Getter<T,
...
class TeOpVarAttrArgsImpl {
 ...
 private:
  static std::map<std::string,
                  std::function<Status(TeOpVarAttrArgsImpl *,
...
std::map<std::string, std::function<Status(TeOpVarAttrArgsImpl *,
                                           const std::string &, DataBuf &)>>
    TeOpVarAttrArgsImpl::data_getter_ = {
        ...
        {"ListUInt16",
         &TeOpVarAttrArgsImpl::GetNodeAttrDataTmpl<uint16_t, true>},
        {"ListUInt32",
         &TeOpVarAttrArgsImpl::GetNodeAttrDataTmpl<uint32_t, true>},
        {"ListUInt64",
         &TeOpVarAttrArgsImpl::GetNodeAttrDataTmpl<uint64_t, true>},
        {"ListFloat", &TeOpVarAttrArgsImpl::GetNodeAttrDataTmpl<float, true>}};

void VarAttrHelper::InitTeOpVarAttr(ge::OpDescPtr &op_desc,
                                    TeOpVarAttrArgs &attr) {
  attr.impl_ = std::make_shared<TeOpVarAttrArgsImpl>(op_desc);
}

Status TeOpVarAttrArgsImpl::GetDataByName(const string &name,
                                          const string &dtype, DataBuf &data) {
...
const uint8_t *TeOpVarAttrArgs::GetData(const std::string &name,
                                        const std::string &dtype,
                                        size_t &size) const {
  DataBuf data(nullptr, 0);
  auto rc = impl_->GetDataByName(name, dtype, data);
  if (rc == domi::SUCCESS) {
    GELOGI("attr found. %s, %s, %p, %ld", name.c_str(), dtype.c_str(),
...
bool FeedTeOpTensorArg(ge::OpDesc::Vistor<ge::GeTensorDescPtr> &tensor_desc,
  ...
  for (auto &desc : tensor_desc) {
    ...
    ge::Format primary_format =
        static_cast<ge::Format>(ge::GetPrimaryFormat(desc->GetFormat()));
    tensor.format = ge::TypeUtils::FormatToSerialString(primary_format);

    tensor.ori_format =
        ge::TypeUtils::FormatToSerialString(desc->GetOriginFormat());
...
void FeedTeOpConstTensor(
    const ge::Node &node, const ge::OpDescPtr &op_desc,
    std::map<std::string, TeConstTensorData> &const_inputs) {
  ge::Operator op =
      ge::OpDescUtils::CreateOperatorFromNode(node.shared_from_this());
...
bool GetCompileInfo(const ge::OpDescPtr &op_desc, const char *op_type,
                    const char *op_name, OpCompileInfo &op_compile_info) {
  bool bres =
      ge::AttrUtils::GetStr(op_desc, COMPILE_INFO_KEY, op_compile_info.key);
  if (!bres) {
    GE_LOGE("Can not find the attribute %s. op_type:%s, op_name:%s",
...
bool GetCompileInfoV2(const ge::OpDescPtr &op_desc, const char *op_type,
                      const char *op_name,
                      optiling::utils::OpCompileInfo &op_compile_info) {
  std::string op_compile_info_key;
  std::string op_compile_info_json;
  bool bres =
      ge::AttrUtils::GetStr(op_desc, COMPILE_INFO_KEY, op_compile_info_key);
  if (!bres) {
  ...
  bres =
      ge::AttrUtils::GetStr(op_desc, COMPILE_INFO_JSON, op_compile_info_json);
  if (!bres) {
    REPORT_CALL_ERROR("E19999",
...
bool GetAtomicCleanCompileInfo(const ge::OpDescPtr &op_desc,
                               const char *op_type, const char *op_name,
                               OpCompileInfo &op_compile_info) {
  bool bres = ge::AttrUtils::GetStr(op_desc, ATOMIC_COMPILE_INFO_KEY,
                                    op_compile_info.key);
  if (!bres) {
    GE_LOGE("Can not find the attribute %s. op_type:%s, op_name:%s",
            ATOMIC_COMPILE_INFO_KEY, op_type, op_name);
    return false;
  }

  bres = ge::AttrUtils::GetStr(op_desc, ATOMIC_COMPILE_INFO_JSON,
                               op_compile_info.str);
  if (!bres) {
    GE_LOGE("Can not find the attribute %s. op_type:%s, op_name:%s",
            ATOMIC_COMPILE_INFO_JSON, op_type, op_name);
    return false;
  }
  return true;
}

bool GetAtomicCleanCompileInfoV2(
    const ge::OpDescPtr &op_desc, const char *op_type, const char *op_name,
    optiling::utils::OpCompileInfo &op_compile_info) {
  std::string op_compile_info_key;
  std::string op_compile_info_json;
  bool bres = ge::AttrUtils::GetStr(op_desc, ATOMIC_COMPILE_INFO_KEY,
  ...
  if (!bres) {
    REPORT_CALL_ERROR("E19999",
                      "Can not find the attribute %s. op_type:%s, op_name:%s",
                      ATOMIC_COMPILE_INFO_KEY, op_type, op_name);
    return false;
  }
  ge::AscendString compile_info_key(op_compile_info_key.c_str());
...
void ParseShapeDesc(const nlohmann::json &shape,
                    std::vector<TeOpTensor> &tensors) {
  TeOpTensor tensor;
...
void ParseShapeDescList(const nlohmann::json &shape_list,
                        std::vector<TeOpTensorArg> &op_args) {
...
template <typename T>
void GetConstDataPointer(const nlohmann::json &json_array,
...
void ParseConstShapeDescV2(
    const nlohmann::json &shape_json, ge::Operator &op_para,
  ...
  ge::GeTensor const_tensor(ge::GeTensorDesc(ge_shape, ge_format, ge_dtype),
                            res.first->second);
  ge::GeTensorPtr const_tensor_ptr =
      std::make_shared<ge::GeTensor>(const_tensor);
  ge::OpDescPtr const_op_desc =
      ge::OpDescUtils::CreateConstOp(const_tensor_ptr);
  ge::Operator const_op =
      ge::OpDescUtils::CreateOperatorFromOpDesc(const_op_desc);
  op_para.SetInput(name, const_op);
  return;
}

void ParseConstShapeDesc(
  ...
  std::transform(format_str.begin(), format_str.end(), format_str.begin(),
                 ::toupper);
  ge::Format ge_format = ge::TypeUtils::SerialStringToFormat(format_str);
  ge::Tensor const_tensor(ge::TensorDesc(ge_shape, ge_format, ge_dtype),
                          res.first->second);
  const_tensors.emplace(
      name, std::make_tuple(const_tensor.GetData(), const_tensor.GetSize(),
                            const_tensor));
  return;
}

void ParseConstTensorListV2(
    const nlohmann::json &shape_list, ge::Operator &operator_para,
    std::map<std::string, std::vector<uint8_t>> &const_values,
    ge::OpDescPtr op_desc) {
  for (const auto &elem : shape_list) {
    if (elem.is_array()) {
...
void ParseConstTensorList(
    const nlohmann::json &shape_list,
    std::map<std::string, TeConstTensorData> &const_tensors,
    std::map<std::string, std::vector<uint8_t>> &const_values) {
  for (const auto &elem : shape_list) {
    if (elem.is_array()) {
...
bool DumpRunInfoV2(optiling::utils::OpRunInfo &run_info, char *run_info_json,
                   size_t run_info_len) {
  ...
  if (str.size() >= run_info_len) {
    REPORT_CALL_ERROR("E19999", "runinfo too large. %zu/%zu", str.size(),
                      run_info_len);
    return false;
  }
  auto rc =
      memcpy_s(run_info_json, str.size() + 1, str.c_str(), str.size() + 1);
  if (rc != EOK) {
    return false;
  }
  return true;
...
bool DumpRunInfo(const OpRunInfo &run_info, char *run_info_json,
                 size_t run_info_len) {
  if (run_info_json == nullptr) {
...
extern "C" int TbeOpTilingPyInterfaceEx2BackUp(
    const char *optype, const char *compile_info, const char *inputs,
  ...
  if (optype == nullptr || compile_info == nullptr || inputs == nullptr ||
      outputs == nullptr) {
    REPORT_CALL_ERROR(
        "E19999", "optype/compile_info/inputs/outputs is null, %s, %s, %s, %s",
  ...
  try {
    ...
    REPORT_CALL_ERROR("E19999", "Failed to parse json_str. %s, %s, %s",
                      compile_info, inputs, outputs);
    return 0;
  }
  GELOGI("Optiling func found, op_type:%s, func:[%s:%p]", optype,
...
void ParseShapeDescV2(const nlohmann::json &shape, ge::OpDescPtr &op_desc,
                      std::string Flag) {
  ...
  if (shape.contains("ori_shape")) {
    tensor.SetOriginShape(
        ge::GeShape(shape["ori_shape"].get<vector<int64_t>>()));
  }
  if (shape.contains("format")) {
  ...
  if (shape.contains("name")) {
    name = shape["name"];
    Flag == "inputs" ? op_desc->AddInputDesc(name, tensor)
                     : op_desc->AddOutputDesc(name, tensor);
  } else {
    Flag == "inputs" ? op_desc->AddInputDesc(tensor)
                     : op_desc->AddOutputDesc(tensor);
  }
}

void ParseShapeDescListV2(const nlohmann::json &shape_list,
                          ge::OpDescPtr &op_desc, std::string Flag) {
  for (const auto &elem : shape_list) {
    if (elem.is_array()) {
      for (const auto &shape : elem) {
...
extern "C" int TbeOpTilingPyInterfaceEx2New(
    const char *optype, const char *compile_info, const char *inputs,
    const char *outputs, char *run_info_json, size_t run_info_len,
    const char *compile_info_hash, uint64_t *elapse,
...
extern "C" int TbeOpTilingPyInterfaceEx2(
    const char *optype, const char *compile_info, const char *inputs,
    const char *outputs, char *run_info_json, size_t run_info_len,
    const char *compile_info_hash, uint64_t *elapse) {
  auto &interf_2 =
      optiling::utils::OpTilingRegistryInterf_V2::RegisteredOpInterf();
  ...
  return (flag == 1 ? TbeOpTilingPyInterfaceEx2New(
                          optype, compile_info, inputs, outputs, run_info_json,
                          run_info_len, compile_info_hash, elapse, iter_2)
                    : TbeOpTilingPyInterfaceEx2BackUp(
                          optype, compile_info, inputs, outputs, run_info_json,
                          run_info_len, compile_info_hash, elapse, iter_1));
}

extern "C" int TbeOpTilingPyInterfaceEx(const char *optype,
                                        const char *compile_info,
                                        const char *inputs, const char *outputs,
                                        char *run_info_json,
                                        size_t run_info_len, uint64_t *elapse) {
  return TbeOpTilingPyInterfaceEx2(optype, compile_info, inputs, outputs,
                                   run_info_json, run_info_len, nullptr,
                                   elapse);
}

extern "C" int TbeOpTilingPyInterface(const char *optype,
                                      const char *compile_info,
                                      const char *inputs, const char *outputs,
                                      char *run_info_json,
                                      size_t run_info_len) {
...
bool StructToClass_RunInfo(OpRunInfo &run_info_struct,
                           optiling::utils::OpRunInfo &run_info_cls) {
  bool res = false;
...
extern "C" ge::graphStatus OpParaCalculate(
    const ge::Node &node, OpRunInfo &run_info,
    std::map<std::string, optiling::OpTilingFunc>::iterator iter) {
  ...
  GELOGI("Do optiling, op_type:%s, op_name:%s", op_type.c_str(),
         op_name.c_str());
  ...
  if (!bres) {
    GE_LOGE("Do optiling, op_type:%s, op_name:%s", op_type.c_str(),
            op_name.c_str());
  ...
  OpCompileInfo op_compile_info;
  bres = GetCompileInfo(op_desc, op_type.c_str(), op_name.c_str(),
                        op_compile_info);
  if (!bres) {
    GE_LOGE("Failed to get compile_info, op_type:%s, op_name:%s",
            op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }

  GELOGI("Optiling func found, op_type:%s, op_name:%s, func:[%s:%p]",
         op_type.c_str(), op_name.c_str(), iter->first.c_str(),
         iter->second.target<OpTilingFuncPtr>());
  bool rc = (iter->second)(op_param, op_compile_info, run_info);
  if (rc) {
    GELOGI("Optiling succeed. op_type:%s, op_name:%s", op_type.c_str(),
           op_name.c_str());
  } else {
    GE_LOGE("Optiling failed. op_type:%s, op_name:%s", op_type.c_str(),
            op_name.c_str());
  }
  return rc ? ge::GRAPH_SUCCESS : ge::GRAPH_FAILED;
}

ge::graphStatus TurnToOpParaCalculate(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info,
    std::map<std::string, optiling::OpTilingFunc>::iterator iter) {
  ...
  if (OpParaCalculate(node, run_info_struct, iter) != ge::GRAPH_SUCCESS) {
    REPORT_CALL_ERROR("E19999",
                      "OpParaCalculate failed, op_type[%s], op_name[%s]",
                      op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }
  if (!StructToClass_RunInfo(run_info_struct, run_info)) {
    REPORT_CALL_ERROR("E19999",
                      "Trans struct to class failed, op_type[%s], op_name[%s].",
                      op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }
...
extern "C" ge::graphStatus OpParaCalculateNew(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info,
    std::map<std::string, optiling::utils::OpTilingFuncV2>::iterator iter) {
  ge::OpDescPtr op_desc = node.GetOpDesc();
  std::string op_type = op_desc->GetType();
  ...
  optiling::utils::OpCompileInfo op_compile_info("", "");
  bool bres = GetCompileInfoV2(op_desc, op_type.c_str(), op_name.c_str(),
                               op_compile_info);
  if (!bres) {
    REPORT_CALL_ERROR("E19999",
                      "Failed to get compile_info, op_type:%s, op_name:%s",
                      op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }
  ...
  if (rc) {
    ...
    REPORT_CALL_ERROR("E19999", "Optiling failed. op_type:%s, op_name:%s",
                      op_type.c_str(), op_name.c_str());
  }
  return rc ? ge::GRAPH_SUCCESS : ge::GRAPH_FAILED;
}

extern "C" ge::graphStatus OpParaCalculateV2(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info) {
  ge::OpDescPtr op_desc = node.GetOpDesc();
  ...
  return (flag == 1 ? OpParaCalculateNew(node, run_info, iter_2)
                    : TurnToOpParaCalculate(node, run_info, iter_1));
}

extern "C" ge::graphStatus OpAtomicCalculate(
    const ge::Node &node, OpRunInfo &run_info,
    std::map<std::string, optiling::OpTilingFunc>::iterator iter) {
  ge::OpDescPtr op_desc = node.GetOpDesc();
  ...
  GELOGI("Do Atomic optiling. op_type:%s, op_name:%s", op_type.c_str(),
         op_name.c_str());
  std::vector<int64_t> atomic_output_indices;
  (void)ge::AttrUtils::GetListInt(op_desc, ge::ATOMIC_ATTR_OUTPUT_INDEX,
                                  atomic_output_indices);
  ...
  GELOGI("Atomic clean size: %ld, op_type:%s, op_name:%s", clean_size,
         origin_op_type.c_str(), op_name.c_str());
  op_param.const_inputs.emplace(
      "workspace_size",
      TeConstTensorData(nullptr, static_cast<size_t>(clean_size),
                        ge::Tensor()));

  OpCompileInfo op_compile_info;
  bool bres = GetAtomicCleanCompileInfo(op_desc, op_type.c_str(),
                                        op_name.c_str(), op_compile_info);
  if (!bres) {
    GE_LOGE("Failed to get compile_info, op_type:%s, op_name:%s",
            op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  ...
  bool rc = (iter->second)(op_param, op_compile_info, run_info);
  if (rc) {
    GELOGI("Atomic optiling succeed. op_type:%s, op_name:%s", op_type.c_str(),
           op_name.c_str());
  } else {
    GE_LOGE("Atomic optiling failed. op_type:%s, op_name:%s", op_type.c_str(),
            op_name.c_str());
  }
...
ge::graphStatus TurnToOpAtomicCalculate(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info,
    std::map<std::string, optiling::OpTilingFunc>::iterator iter) {
  OpRunInfo run_info_struct;
  run_info_struct.block_dim = run_info.GetBlockDim();
  ...
  if (OpAtomicCalculate(node, run_info_struct, iter) != ge::GRAPH_SUCCESS) {
    REPORT_CALL_ERROR("E19999",
                      "OpAtomicCalculate failed, op_type[%s], op_name[%s]",
                      op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }
  if (!StructToClass_RunInfo(run_info_struct, run_info)) {
...
extern "C" ge::graphStatus OpAtomicCalculateNew(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info,
    std::map<std::string, optiling::utils::OpTilingFuncV2>::iterator iter) {
  ge::OpDescPtr op_desc = node.GetOpDesc();
  std::string op_type = "DynamicAtomicAddrClean";
  ...
  if (atomic_output_indices.empty()) {
    REPORT_CALL_ERROR(
        "E19999", "No ATOMIC_ATTR_OUTPUT_INDEX found, op_type:%s, op_name:%s",
        origin_op_type.c_str(), op_name.c_str());
    return ge::GRAPH_FAILED;
  }
  ge::GeTensorDescPtr tensor =
      op_desc->MutableOutputDesc(atomic_output_indices[0]);
  if (tensor == nullptr) {
    REPORT_CALL_ERROR("E19999",
  ...
  optiling::utils::OpCompileInfo op_compile_info("", "");
  bool bres = GetAtomicCleanCompileInfoV2(op_desc, op_type.c_str(),
                                          op_name.c_str(), op_compile_info);
  if (!bres) {
    REPORT_CALL_ERROR("E19999",
                      "Failed to get compile_info, op_type:%s, op_name:%s",
  ...
  if (rc) {
    ...
    REPORT_CALL_ERROR("E19999",
                      "Atomic optiling failed. op_type:%s, op_name:%s",
                      op_type.c_str(), op_name.c_str());
  }
  return rc ? ge::GRAPH_SUCCESS : ge::GRAPH_FAILED;
}

extern "C" ge::graphStatus OpAtomicCalculateV2(
    const ge::Node &node, optiling::utils::OpRunInfo &run_info) {
  ge::OpDescPtr op_desc = node.GetOpDesc();
  std::string optype = op_desc->GetType();
  auto &interf_2 =
  ...
  return (flag == 1 ? OpAtomicCalculateNew(node, run_info, iter_2)
                    : TurnToOpAtomicCalculate(node, run_info, iter_1));
}
}  // namespace optiling
