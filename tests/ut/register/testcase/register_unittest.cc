...
TEST_F(UtestRegister, tik2_py_interface_get_replay_config_ok) {
  ...
  std::string res_info(1024, 'a');
  size_t size = 1024;
  REG_REPLAY_CONFIG(tik2_py_interface_get_replay_config_ok, replay_config_stub);
  EXPECT_EQ(Tik2PyInterfaceGetReplayConfig(op_type.c_str(), const_cast<char *>(res_info.c_str()), size),
            1);
...
TEST_F(UtestRegister, tik2_py_interface_get_replay_config_fail_without_params) {
  setenv("ENABLE_RUNTIME_V2", "1", 0);
  EXPECT_EQ(Tik2PyInterfaceGetReplayConfig(nullptr, nullptr, 0), 0);
  unsetenv("ENABLE_RUNTIME_V2");
}