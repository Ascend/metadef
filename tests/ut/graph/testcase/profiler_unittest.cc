...
TEST_F(ProfilerUt, OneRecord) {
  ...
  p->Record(0, 1, 2, kEventStart, std::chrono::system_clock::now());
...
TEST_F(ProfilerUt, TimeStampRecord) {
  auto p = Profiler::Create();
  p->Record(0, 1, 2, kEventTimestamp, std::chrono::system_clock::now());
...
TEST_F(ProfilerUt, MultipleRecords) {
  auto p = Profiler::Create();
  p->Record(0, 1, 2, kEventStart, std::chrono::system_clock::now());
  p->Record(0, 1, 2, kEventEnd, std::chrono::system_clock::now());
...
TEST_F(ProfilerUt, RecordCurrentThread) {
  auto p = Profiler::Create();
  p->RecordCurrentThread(0, 2, kEventStart, std::chrono::system_clock::now());
...
TEST_F(ProfilerUt, RegisterStringBeyondMaxSize) {
  ...
  p->Record(kMaxStrIndex, 1, 2, kEventStart, std::chrono::system_clock::now());
...
TEST_F(ProfilerUt, EventTypeBeyondRange) {
  auto p = Profiler::Create();
  p->Record(0, 1, 2, kEventTypeEnd, std::chrono::system_clock::now());
