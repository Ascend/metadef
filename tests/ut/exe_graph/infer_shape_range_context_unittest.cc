...
TEST_F(InferShapeRangeContextUT, GetInputShapeRangeOk) {
  ...
  gert::ShapeRange in_shape_range1(&same_ele);
  Shape min1{2, 2, 3, 8};
  Shape max1{2, -1, 3, 8};
  gert::ShapeRange in_shape_range2(&min1, &max1);
  Shape out_shape1{8, 3, 224, 224};
  Shape out_shape2{8, 224, 224, 224};
  gert::ShapeRange out_shape_range(&out_shape1, &out_shape2);
...
TEST_F(InferShapeRangeContextUT, GetDynamicInputShapeRangeOk) {
  ...
  gert::ShapeRange in_shape_range1(&min1, &max1);
  Shape min2{2, 2, 3, 8};
  Shape max2{2, -1, 3, 8};
  gert::ShapeRange in_shape_range2(&min2, &max2);
  Shape min3{3, 2, 3, 8};
  Shape max3{3, 2, 9, 8};
  gert::ShapeRange in_shape_range3(&min3, &max3);
  Shape min4{4, 2, 3, 8};
  Shape max4{4, 2, 3, 16};
  gert::ShapeRange in_shape_range4(&min4, &max4);
  Shape min5{8, 3, 224, 224};
  Shape max5{-1, 3, 224, 224};
  gert::ShapeRange out_shape_range(&min5, &max5);
  auto context_holder = InferShapeRangeContextFaker()
...
TEST_F(InferShapeRangeContextUT, GetOutShapeOk) {
  ...
  gert::ShapeRange out_shape_range(&out_min, &out_max);
  auto context_holder = InferShapeRangeContextFaker()
      .IrInputNum(2)
      .NodeIoNum(2, 1)
      .NodeInputTd(0, ge::DT_FLOAT16, ge::FORMAT_NCHW, ge::FORMAT_NC1HWC0)
